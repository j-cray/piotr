use anyhow::Context;
use dotenv::dotenv;
use piotr::{ai, bot, config::AppConfig, db, signal, utils};
use std::sync::Arc;
...
async fn main() -> anyhow::Result<()> {
    ...
    let profile_manager = ai::memory::DbProfileManager::new(
        db.pool.clone(),
    ...
    let (signal_client, mut rx) =
        match signal::SignalClient::new(&signal_phone, &config.signal.data_path).await {
            Ok(res) => {
    ...
    let session_manager = bot::SessionManager::new(
        signal_client,
        ai_client,
        bot_number,
        profile_manager,
    ...
    process_message_stream(
        ...
        move |envelope| {
            ...
            async move { sm.handle_message(envelope).await }
        },
    )
    .await;
...
mod tests {
    use super::*;
    use piotr::signal::{Envelope, SignalMessage};
    use std::sync::atomic::{AtomicUsize, Ordering};

    #[tokio::test]
    async fn test_process_message_stream_concurrency() {
        unsafe {
            std::env::set_var("ANONYMIZE_KEY", "test_key");
        }
        ...
        let handler = move |_env: Envelope| {
            ...
            async move {
                ...
                while current > current_max {
                    if max
                        .compare_exchange(current_max, current, Ordering::SeqCst, Ordering::SeqCst)
                        .is_ok()
                    {
                        break;
                    }
        ...
        assert_eq!(
            ...
            max_concurrent,
            "Did not properly restrict concurrency"
        );
    }
    ...
    async fn test_process_message_stream_timeout() {
        ...
        assert_eq!(
            ...
            0,
            "Handler should have been timed out before completion"
        );
    }
}
