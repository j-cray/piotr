...
export PGDATA=$(pwd)/data/db

if command -v initdb &> /dev/null; then
    if [ ! -d "$PGDATA" ]; then
        echo "Initializing PostgreSQL data directory at $PGDATA..."
        # Use the secure password for new init
        initdb -D "$PGDATA" --auth=md5 --pwfile=<(echo "$SECURE_PASS") --username="$CURRENT_USER"
    ...
    if ! pg_ctl status -D "$PGDATA" &> /dev/null; then
        ...
        pg_ctl start -D "$PGDATA" -l "$PGDATA/logfile" -o "-p 5432 -k '$PGDATA'"
        sleep 2
    fi
else
...
export PGPASSWORD="$SECURE_PASS"
if psql -U "$DB_USER" -d postgres -c "\q" &> /dev/null; then
...
if grep -q "DATABASE_URL" .env; then
    ...
    ESCAPED_URL="postgres://${DB_USER}:${SECURE_PASS}@localhost/${DB_NAME}"
    ...
    echo "DATABASE_URL=postgres://${DB_USER}:${SECURE_PASS}@localhost/${DB_NAME}" >> .env
